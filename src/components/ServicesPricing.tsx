...
const SERVICE_CATEGORIES: ServiceCategory[] = [
  {
    ...
    alt: 'Cắt và tạo kiểu tóc tại No.1 Hair Room',
    women: [
      {
  ...
  },
  {
    ...
    alt: 'Uốn và duỗi tóc tại No.1 Hair Room',
    women: [
      {
        title: 'Uốn · Duỗi',
        services: [
  ...
  },
  {
    ...
    alt: 'Nhuộm và tạo màu tóc tại No.1 Hair Room',
    women: [
      {
  ...
  },
  {
    ...
    alt: 'Chăm sóc và phục hồi tóc tại No.1 Hair Room',
    women: [
      {
  ...
  return (
    <div className="flex flex-col gap-8">
      {groups.map((group) => (
        <div key={group.title}>
          <h4
            className="mb-4 text-[10px] uppercase tracking-[0.24em] text-[#767676]"
          ...
          <div className="divide-y divide-[#1C1C1C]/10">
            {group.services.map((service) => (
              <div key={service.name} className="flex items-baseline gap-3 py-3 first:pt-0">
                <span
                  className="min-w-0 text-[15px] leading-[1.55] text-[#1C1C1C] md:text-[16px]"
                  style={{ fontFamily: "'Inter', sans-serif" }}
                >
                  {service.name}
                </span>
                <span
                  className="h-px flex-1 translate-y-[-3px] border-b border-dotted border-[#1C1C1C]/20"
                  aria-hidden="true"
                />
                <span
                  className="shrink-0 text-right text-[12px] font-medium text-[#1C1C1C] md:text-[13px]"
                  style={{ fontFamily: "'JetBrains Mono', monospace" }}
                >
                  {service.price}
                </span>
...
function AudienceMenu({ label, groups }: { label: string; groups: ServiceGroup[] }) {
  ...
  return (
    <div className="rounded-[2px] border border-[#1C1C1C]/10 bg-white/55 p-6 md:p-8">
      <h3
        className="mb-7 text-[24px] tracking-[-0.03em] text-[#1C1C1C] md:text-[28px]"
...
function CategoryMenu({ category }: { category: ServiceCategory }) {
  return (
    <article id={category.id} className="scroll-mt-28 border-t border-[#1C1C1C]/15 pt-12 md:pt-16">
      <div className="mb-8 flex items-end justify-between gap-6 md:mb-10">
        <div>
          <span
            className="text-[10px] tracking-[0.28em] text-[#767676]"
            style={{ fontFamily: "'JetBrains Mono', monospace" }}
          >
            {category.number}
          </span>
          <h3
            className="mt-3 text-[32px] leading-none tracking-[-0.045em] text-[#1C1C1C] md:text-[48px]"
        ...
        <a
          ...
          onClick={(e) => scrollToId(e, 'services-pricing')}
          className="hidden text-[10px] uppercase tracking-[0.2em] text-[#767676] transition-colors hover:text-[#1C1C1C] md:block"
          style={{ fontFamily: "'JetBrains Mono', monospace" }}
        >
          Về đầu trang
        </a>
        <a
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full border border-[#1C1C1C]/15 text-[#1C1C1C] transition-colors hover:border-[#2A2A2A] hover:text-[#2A2A2A] md:hidden"
        >
    ...
    <section id="services-pricing" aria-label="Services and pricing" className="bg-[#F6F6F5] py-20 md:py-28">
      <div className="container mx-auto px-6 sm:px-10 lg:px-16">
        <div className="max-w-[620px]">
          ...
          <p
            className="mt-7 max-w-[500px] text-[14px] leading-[1.8] text-[#767676] md:text-[15px]"
            style={{ fontFamily: "'Inter', sans-serif" }}
          >
            Bảng giá cắt, uốn, nhuộm, duỗi và phục hồi tóc tại No.1 Hair Room, Thủ Dầu Một, Bình Dương. Mỗi dịch vụ được thực hiện tỉ mỉ với mức giá hợp lý, phù hợp cho cả khách hàng mới và khách quen.
          </p>
        </div>
        ...
        <nav aria-label="Danh mục dịch vụ" className="mt-14 grid grid-cols-2 gap-3 md:mt-20 md:grid-cols-4 md:gap-4">
          {SERVICE_CATEGORIES.map((category) => (
            <a
              key={category.id}
              href={`#${category.id}`}
              className="group relative block aspect-[3/4] overflow-hidden bg-[#1C1C1C] text-white"
            >
              <img
                src={category.image}
                alt={category.alt}
        ...
        <nav aria-label="Đi đến danh mục giá" className="mt-10 flex gap-x-6 gap-y-3 overflow-x-auto border-y border-[#1C1C1C]/10 py-5 md:mt-14 md:justify-center">
          {SERVICE_CATEGORIES.map((category) => (
            <a
              key={category.id}
              href={`#${category.id}`}
              className="shrink-0 text-[10px] uppercase tracking-[0.15em] text-[#767676] transition-colors hover:text-[#1C1C1C]"
              style={{ fontFamily: "'JetBrains Mono', monospace" }}
            >
